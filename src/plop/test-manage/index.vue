...
// api
import { addTest, deleteTest, updateTest } from '@/api/test'
...
export default {
  ...
  async created() {
    this.tableConfig = getTableConfig(
      ...
      (row) => {
        this.removeByCode(row.test_code)
      }
    )
    if (!this.metadata) {
  ...
  methods: {
    ...
    async addOrUpdateList(params, code) {
      const res = await (this.addOperation ? addTest(params) : updateTest(code, params))
    ...
    async removeByCode(code) {
      ...
      if (confirmResult !== 'confirm') {
        ...
        const res = await deleteTest(code)
